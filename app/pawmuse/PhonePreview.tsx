import { ArrowLeft, Camera, ChevronDown, Heart, Home, MapPin, MessageCircle, MoreHorizontal, Search, Share2, ShoppingBag, Star, UserRound } from "lucide-react";
import { BRAND, IMAGES, WECHAT_COPY, XHS_NOTE, XHS_POSTS } from "./data";
import { COMPLETE_STORY_PROGRESS, type Platform, type StoryProgress } from "./types";
...
function MeituanScreen({ progress }: { progress: StoryProgress }) {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-y-auto rounded-[30px] bg-[#F5F5F5] text-[#111] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      ...
      <div className="relative w-full overflow-hidden bg-white">
        <img src={IMAGES.meituanCover} alt="爱宠门店主图" className="block h-auto w-full" />
      ...
      <div className="relative z-10 -mt-6 px-3 pb-16">
        <div className="rounded-xl bg-white p-4 shadow-[0_3px_14px_#00000010]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0"><h3 className="text-[16px] font-extrabold">{BRAND.name}</h3><p className="mt-1 truncate text-[10px] text-[#777]">{BRAND.positioning} · 距您500m</p></div>
        ...
        <div className="rounded-b-xl bg-white px-4 pb-4">
          ...
          {progress.meituanService ? <div className="flex gap-3">
            <img src={IMAGES.works[0]} alt="专属皮毛调理洗护服务图" className="size-20 shrink-0 rounded-md border border-[#FFD0C4] object-cover" />
...
function XiaohongshuProfile({ onOpenPost, progress }: { onOpenPost?: () => void; progress: StoryProgress }) {
  if (!progress.xhsProfile) return <EmptyPlatformScreen label="小红书" />;
  return (
    <div className="relative h-full overflow-hidden rounded-[30px] bg-white text-[#222]">
      ...
      <div className="relative h-[325px] overflow-hidden bg-[#181818] text-white">
        <img src={IMAGES.xhsCover} alt="小红书主页背景" className="absolute inset-0 h-full w-full object-cover opacity-45" />
        ...
        <div className="absolute inset-x-4 bottom-5">
          <div className="flex items-center gap-3">
            <div className="grid size-16 place-items-center overflow-hidden rounded-full border-2 border-white bg-white"><img src={IMAGES.logo} alt="爱宠 logo" className="h-full w-full object-cover" /></div>
            <div><h3 className="text-[16px] font-bold">{BRAND.name}</h3><p className="mt-1 text-[9px] text-white/65">小红书号：88888888</p></div>
          </div>
          <p className="mt-3 text-[10px] text-white/85">{BRAND.slogan}</p>
          <div className="mt-3 flex gap-2">{['宠物SPA', '高端养宠', '一宠一浴'].map(tag => <span key={tag} className="rounded-full bg-white/10 px-2 py-1 text-[8px]">{tag}</span>)}</div>
      ...
      {progress.xhsPoster ? <div className="grid grid-cols-2 gap-x-1.5 gap-y-3 p-1.5">
        {XHS_POSTS.slice(0, 1).map((post, index) => (
          <button key={post.title} type="button" onClick={index === 0 ? onOpenPost : undefined} className="overflow-hidden rounded-md bg-white text-left">
            <div className={`overflow-hidden bg-[#EEF1ED] ${index % 3 === 0 ? "h-40" : "h-32"}`}><img src={post.image} alt="笔记封面" className="h-full w-full object-cover" /></div>
            <div className="p-2"><p className="line-clamp-2 text-[10px] font-semibold leading-4">{post.title}</p><div className="mt-2 flex items-center justify-between text-[8px] text-[#888]"><span className="flex items-center gap-1"><span className="grid size-4 place-items-center rounded-full bg-[#F4E2D8]">爱</span>{BRAND.name}</span><span>♡ {post.likes}</span></div></div>
          </button>
        ))}
...
function XiaohongshuNote({ onBack }: { onBack?: () => void }) {
  return (
    <div className="relative h-full overflow-y-auto rounded-[30px] bg-white text-[#222] [scrollbar-width:none]">
      ...
      <div className="sticky top-0 z-20 mt-7 flex h-[60px] items-center justify-between border-b border-[#EEE] bg-white px-4">
        <button type="button" onClick={onBack} aria-label="返回小红书主页" className="grid size-8 place-items-center rounded-full transition hover:bg-[#F5F5F5]"><ArrowLeft size={19} /></button><div className="flex flex-1 items-center gap-2 pl-2"><img src={IMAGES.logo} alt="logo" className="size-8 rounded-full object-contain" /><span className="text-[11px] font-semibold">{BRAND.name}</span></div><button className="rounded-full bg-[#FF2442] px-4 py-2 text-[10px] font-bold text-white">关注</button><MoreHorizontal size={18} className="ml-3" />
      </div>
      <div className="relative w-full overflow-hidden bg-[#ECEBE8]"><img src={IMAGES.xhsPoster} alt="小红书笔记大图" className="block h-auto w-full" /></div>
      <div className="px-4 pb-20"><h2 className="text-[15px] font-bold leading-6">✨ 商场里藏着一家宠物 SPA，终于找到宠物的“高级护理空间”🐾</h2><p className="mt-3 whitespace-pre-line text-[11px] leading-[1.75]">{XHS_NOTE}</p><p className="mt-4 border-t border-[#EEE] pt-3 text-[9px] text-[#AAA]">编辑于 1分钟前 · 上海</p></div>
...
function WechatScreen({ progress }: { progress: StoryProgress }) {
  ...
  return (
    <div className="relative h-full overflow-y-auto rounded-[30px] bg-white text-[#222] [scrollbar-width:none]">
      ...
      <div className="relative h-[325px] overflow-hidden bg-gradient-to-br from-[#4DB6C8] to-[#65BF77]">
        <img src={IMAGES.wechatCover} alt="微信朋友圈封面" className="absolute inset-0 h-full w-full object-cover" />
        <div className="absolute left-4 right-4 top-9 flex justify-between text-white"><ArrowLeft size={18} /><Camera size={19} /></div>
        <div className="absolute -bottom-5 right-4 flex items-end gap-3"><span className="mb-7 text-[13px] font-bold text-white">{BRAND.name}</span><img src={IMAGES.logo} alt="头像" className="size-16 rounded-lg border-2 border-white bg-white object-contain shadow" /></div>
      </div>
      {progress.wechatPoster ? <div className="px-4 pb-4 pt-9">
        <div className="flex gap-3"><img src={IMAGES.logo} alt="头像" className="size-9 rounded-md bg-white object-contain" /><div className="min-w-0 flex-1"><div className="text-[11px] font-bold text-[#576B95]">{BRAND.name}</div><p className="mt-1 whitespace-pre-line text-[10px] leading-[1.6]">{WECHAT_COPY}</p><img src={IMAGES.wechatPoster} alt="朋友圈活动海报" className="mt-2 w-44 rounded-md object-contain" /><div className="mt-2 flex justify-between text-[8px] text-[#AAA]"><span>1分钟前</span><MoreHorizontal size={14} className="rounded bg-[#F3F3F3] px-0.5 text-[#576B95]" /></div></div></div>
...
export function PhonePreview({ platform, xhsDetail = false, compact = false, onOpenXhsPost, onBackXhsPost, progress = COMPLETE_STORY_PROGRESS }: PhonePreviewProps) {
  const screen = platform === "meituan" ? <MeituanScreen progress={progress} /> : platform === "wechat" ? <WechatScreen progress={progress} /> : xhsDetail && progress.xhsPoster ? <XiaohongshuNote onBack={onBackXhsPost} /> : <XiaohongshuProfile onOpenPost={onOpenXhsPost} progress={progress} />;
